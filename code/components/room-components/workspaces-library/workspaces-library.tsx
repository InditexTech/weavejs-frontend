...
export const WorkspacesLibrary = () => {
  ...
  const workspacesAvailable = React.useMemo(() => {
    ...
    const nodes = instance.findNodesByType(appState.weave as WeaveStateElement, "workspace");
  ...
  const alignItemsHandler = React.useCallback(() => {
    ...
    instance.triggerAction<{ gap: number; nodes: WeaveSelection[] }>("alignElementsTool", {
      gap: 20,
      nodes: selectedNodes,
    });
  ...
  const exportWorkspacesHandler = React.useCallback(async () => {
    ...
    if (workspacesToRender.length === 0) {
      workspacesToRender = workspacesAvailable.map((e) => e.getAttrs().id ?? "");
    }
    ...
    <div className="pointer-events-auto w-full h-full">
      ...
      <div className="flex flex-col gap-2 w-full h-[calc(100%-53px)] p-4 overflow-scroll">
        ...
        {workspacesAvailable.map((node) => {
          ...
          return (
            <div key={attrs.id} className="w-full bg-light-background-1 flex flex-col gap-2">
              <div className="w-full flex justify-between items-center">
                <div className="w-full font-label-l-regular">{attrs.title}</div>
                <div className="font-label-l-regular">
                  <input
                    type="checkbox"
                    checked={selectedWorkspaces.findIndex((e) => e === attrs.id) !== -1}
                    onChange={() => {
